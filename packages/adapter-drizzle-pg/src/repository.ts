...
import {
  eq, and, isNull, isNotNull, inArray, sql, asc, desc,
  gt, lt, or,
} from 'drizzle-orm'
...
export class DrizzleRepository implements IRepository<Record<string, unknown>> {
  ...
  constructor(options: DrizzleRepositoryOptions) {
    ...
    this._columns = (this._table as Record<string, unknown>)[Symbol.for('drizzle:Columns')] as Record<string, PgColumn>
      ?? Object.fromEntries(
  ...
  async transaction<TManager = unknown>(
    ...
    try {
      return await (this._db as PostgresJsDatabase).transaction(async (tx) => {
  ...
  private getColumn(name: string): PgColumn | undefined {
    return this._columns[name] ?? (this._table as Record<string, unknown>)[name] as PgColumn | undefined
  }
