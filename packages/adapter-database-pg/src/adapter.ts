...
export class DrizzlePgAdapter implements IDatabasePort {
  ...
  /**
   * Set the full schema (tables + relations) to enable Drizzle's relational query API.
   * After this call, `db.query.*` is available for findMany/findFirst with `with` clauses.
   */
  setSchema(schema: Record<string, unknown>): void {
    ...
    this._db = drizzle(this._sql, { schema }) as unknown as PostgresJsDatabase
  }
  ...
  async transaction<T>(fn: (tx: unknown) => Promise<T>, options?: TransactionOptions): Promise<T> {
