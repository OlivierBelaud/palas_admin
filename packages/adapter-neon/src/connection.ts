// Neon connection adapter — uses @neondatabase/serverless WebSocket Pool for fast queries
...
export function createNeonDatabase(options: NeonDatabaseOptions): NeonDatabase {
  ...
  // Serverless + Neon → use WebSocket Pool (reuses connection, ~20ms/query vs ~250ms/query HTTP)
  if (isServerless && isNeon) {
    // WebSocket is a global in Node 18+ and Vercel runtime
