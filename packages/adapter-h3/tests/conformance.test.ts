...
describe('H3Adapter — IHttpPort conformance', () => {
  ...
  it('MantaError(INVALID_DATA) → 400', async () => {
    ...
    const res = await adapter.handleRequest(new Request('http://localhost/api/products', { method: 'POST' }))
...
describe('H3Adapter — HTTP server lifecycle', () => {
  it('listen and close lifecycle', async () => {
    ...
    // Note: port 0 means OS assigns random port, but H3Adapter uses fixed port
    // So we skip the actual network test and just verify the lifecycle
    await adapter.close()
  })
})

describe('H3Adapter — getApp()', () => {
  it('returns the underlying H3 App', () => {
    const adapter = new H3Adapter({ port: 0, isDev: true })
