...
describe('Pipeline — Step 6: Auth', () => {
  ...
  it('HP-03 — valid auth token injects authContext', async () => {
    ...
    expect(res.status).toBe(200)
    const data = await res.json()
    expect(data.auth).toEqual({ type: 'user', id: 'user_123' })
  })

  // HP-04 — /api/admin/* without auth returns 401 when context auth is registered
  it('HP-04 — /api/admin/ without auth returns 401 when context auth registered', async () => {
    adapter.setAuthVerifier(async () => null)
    adapter.registerContextAuth('admin', 'admin', ['/api/admin/login'])

    adapter.registerRoute('GET', '/api/admin/protected', async () => {
      return Response.json({ secret: true })
    })

    const res = await adapter.handleRequest(new Request('http://localhost/api/admin/protected'))
  ...
  it('HP-05 — /store/ without auth succeeds (auth is optional)', async () => {
    ...
    const res = await adapter.handleRequest(new Request('http://localhost/store/products'))
    expect(res.status).toBe(200)
  })

  // HP-06 — Invalid token on /api/admin returns 401
  it('HP-06 — invalid token on /api/admin returns 401', async () => {
    adapter.setAuthVerifier(async () => null)
    adapter.registerContextAuth('admin', 'admin', ['/api/admin/login'])

    adapter.registerRoute('GET', '/api/admin/data', async () => {
  ...
  it('HP-07 — no authVerifier = auth step skipped, request proceeds', async () => {
    ...
    const res = await adapter.handleRequest(new Request('http://localhost/admin/data'))
    expect(res.status).toBe(200)
  })
})
        ...
        bodySchema: {
          parse: (data: unknown) => {
            ...
            if (!d || !d.name) {
              ...
              ;(err as unknown as { issues: unknown[] }).issues = [{ path: ['name'], message: 'Required' }]
              throw err
            }
            return d
          },
...
describe('Pipeline — Step 10: RBAC (basic namespace check)', () => {
  ...
  it('HP-10 — admin actor on /admin route passes RBAC', async () => {
    adapter.setAuthVerifier(async () => ({ type: 'user', id: 'admin_1' }))
