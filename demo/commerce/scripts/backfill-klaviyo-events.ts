...
async function hogql(query: string): Promise<unknown[][]> {
  return await runPosthogHogQL(query, { privateKey: PH_KEY, refresh: 'force_blocking' })
}
...
try {
  ...
  let totalInserted = 0
  let totalScanned = 0
  const [latest] = await sql<Array<{ max_at: Date | null }>>`
    SELECT MAX(occurred_at) AS max_at FROM klaviyo_events
  `
  const since = latest?.max_at
    ? new Date(new Date(latest.max_at).getTime() - 3600 * 1000)
    : new Date(Date.now() - 365 * 86400 * 1000)
  const sinceIso = since.toISOString()
  console.log(`[backfill-klaviyo-events] since=${sinceIso}`)

  for (let windowStartMs = since.getTime(); windowStartMs < Date.now(); windowStartMs += DAY_MS) {
    const windowStart = new Date(windowStartMs).toISOString()
    const windowEnd = new Date(Math.min(windowStartMs + DAY_MS, Date.now())).toISOString()
    offset = 0
    ...
    while (true) {
      ...
      WHERE ke.datetime >= '${hogqlDateString(windowStart)}'
        AND ke.datetime < '${hogqlDateString(windowEnd)}'
        AND lower(kp.email) != ''
        AND (
