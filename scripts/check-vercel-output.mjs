...
export async function checkVercelOutput(workspaceRoot = '.') {
  const fromRoot = (path) => resolve(workspaceRoot, path)
  const outputRoot = fromRoot('demo/commerce/.vercel/output')
  const manifest = readJson(fromRoot('demo/commerce/vercel-fast-functions.manifest.json'))
  const runtimeContracts = readJson(fromRoot('demo/commerce/runtime-contracts.json'))
  const customRoutes = manifest.functions.map((spec) => spec.route)
  const expectedFunctions = ['__server', ...customRoutes]
  const sourceDir = fromRoot('demo/commerce/vercel-fast-functions')
  ...
  if (!isDeepStrictEqual(deployableSources, manifestSources)) {
    ...
    throw new Error(
      `Vercel source inventory drift (missing: ${missing.join(', ') || 'none'}; stale: ${stale.join(', ') || 'none'})`,
    )
  }
  ...
  if (!isDeepStrictEqual(actualFunctions, sortedExpectedFunctions)) {
    ...
    throw new Error(
      `Vercel function inventory drift (missing: ${missing.join(', ') || 'none'}; unexpected: ${unexpected.join(', ') || 'none'})`,
    )
  }
  ...
  for (const route of expectedFunctions) {
    ...
    if (config.launcherType !== 'Nodejs') throw new Error(`${route} must use the explicit Node launcher`)
    if (typeof config.handler !== 'string' || !config.handler || !existsSync(`${dir}/${config.handler}`)) {
      throw new Error(`${route} is missing configured handler ${config.handler ?? '(missing)'}`)
    }

    // Nitro's fallback is a Node launcher, not a fetch-style fast function.
    if (route === '__server') continue
    if (config.handler !== 'index.mjs') throw new Error(`${route} must use index.mjs handler`)
